...
/**
 * Provides the resources for geolocating devices based on neighboring cell measurement reports
 */
export class NcellmeasGeolocation extends CloudFormation.Resource {
	public constructor(
				...
				fromNrfConnectForCloud = new Lambda.Function(
					...
					'fromNrfConnectForCloud',
					{
						...
						code: lambdas.lambdas
							.ncellmeasGeolocationFromNrfConnectForCloudStepFunction,
						description:
		...
		new StepFunctions.StateMachine(this, 'StateMachine', {
			...
			definition: (() => {
				...
				return new StepFunctions.Parallel(
					...
					.branch(...branches)
					.next(
						new StepFunctions.Choice(this, 'Resolved from any third party API?')
							.when(...checkApiResult(0))
							.when(...checkApiResult(1))
							.otherwise(
								new StepFunctions.Pass(this, 'no: mark cell as not located', {
									resultPath: '$.ncellmeasgeo',
