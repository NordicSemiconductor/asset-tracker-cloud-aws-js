...
export type ConcatenateFilesFn = (args: ConcatenateFilesArgs) => ConcatenateFilesResult

export const concatenateFiles = ({ Bucket,
    s3 }: { Bucket: string, s3: S3 }) => async ({
        dateToFileName,
        files,
    }: ConcatenateFilesArgs): ConcatenateFilesResult => Object.entries(files).reduce(async (p, [date, files]) => {
        await p
        // Fetch all bodies
